  ...
  if os.path.exists(NOTEPAD_FILE):
    try:
      with open(NOTEPAD_FILE, "r") as f:
        data = json.load(f)
        user_data = data.get(
            encode_cred(username), {"current": "", "history": []}
        )
...
def track_user_activity(username, action="login"):
  ...
  if u_key not in metrics:
    metrics[u_key] = {
        ...
        "license_expiry": (
            datetime.now(ZoneInfo("Africa/Lagos")) + timedelta(days=7)
...
# ==========================================
# MULTILINGUAL DICTIONARY MATRIX WITH ABOUT & SPORTY/FOREX
# ==========================================
LANG_DATA = {
    "English": {
        ...
        "signin": "🔐 Sign In",
        "register": "🛠️ Register New Workspace",
        "username": "Security ID Username",
        "password": "Access Key Password",
        "btn_auth": "Authorize Access",
        "btn_reg": "Provision Hidden Profile Space",
        "nav_lbl": "Select Operation Target Engine:",
        "clear_cache": "Clear Cache Data 🗑️",
        "logout": "Terminate Session 🔓",
        "search_title": "🌐 Global Commodity Cost & Currency Conversion Oracle",
        "search_btn": "🔍 Look Up Commodity Cost",
        "search_ph": (
            "e.g., 1 bag of cement, barrel of crude oil, price of rice per ton"
        ),
        "curr_lbl": "Target Currency Symbol:",
        "upload_lbl": "Upload Corporate Knowledge Assets (PDF)",
        "orac_res": "🌐 Pricing Oracle Result:",
        "comp_panel": "📊 Company Progress Panel",
        "oracle_chat": "💬 Context Oracle Chat",
        "exec_brief": "📝 Executive Briefing Engine",
        "composer": "✉️ Contextual Draft Composer",
        "extractor": "📊 Structural Data Extractor",
        "cross_file": "🔍 Cross-File Intelligence",
        "tracker": "📅 Task & Milestone Tracker",
        "sandbox": "💡 Brainstorm Sandbox",
        "predictor": "💸 Resource Predictor",
        "indexer": "🗂️ Knowledge Base Indexer",
        "runway_plan": "🛡️ Runway & Reinvestment Planner",
        "sporty_ai": "⚽ Sporty Teaching AI",
        "forex_ai": "📈 Forex Teaching AI",
        "about_panel": "ℹ️ About Application",
        "run_diag": "⚡ Run System Diagnostic Engine",
        "error_ingest": (
            "🚨 Ingestion Missing: Drop asset documents into the workspace"
            " first."
        ),
        "calc_btn": "📊 Calculate Parameters",
        "success_ac": "⚡ Data Accelerator updated successfully!",
        "save_fn": "Save As (Filename):",
        "dl_lbl": "📥 Save As (Download File)",
        "metrics_lbl": "Enter comma-separated metric values:",
    },
    "Français": {
        "title": "⚡ VEKTOR.AI // Matrice de Nœuds Multi-Agents",
        "gate_title": "SHELL LOCAL SÉCURISÉ — PORTAIL D'ESPACE PERSONNEL",
        "signin": "🔐 Se Connecter",
        "register": "🛠️ Enregistrer un Espace",
        "username": "Nom d'utilisateur de Sécurité",
        "password": "Mot de Passe d'Accès",
        "btn_auth": "Autoriser l'Accès",
        "btn_reg": "Créer un Espace Masqué",
        "nav_lbl": "Sélectionner le Moteur Opérationnel:",
        "clear_cache": "Effacer le Cache Data 🗑️",
        "logout": "Terminer la Session 🔓",
        "search_title": (
            "🌐 Oracle des Coûts des Matières Premières & Conversion"
        ),
        "search_btn": "🔍 Chercher le Coût",
        "search_ph": (
            "ex. 1 sac de ciment, baril de pétrole brut, prix du riz au tonne"
        ),
        "curr_lbl": "Symbole Devise Cible:",
        "upload_lbl": "Charger des Documents de Connaissance d'Entreprise (PDF)",
        "orac_res": "🌐 Résultat de l'Oracle de Prix:",
        "comp_panel": "📊 Panneau de Progrès d'Entreprise",
        "oracle_chat": "💬 Chat Contexte Oracle",
        "exec_brief": "📝 Moteur de Briefing Exécutif",
        "composer": "✉️ Compositeur de Projet Contextuel",
        "extractor": "📊 Extracteur de Données Structurelles",
        "cross_file": "🔍 Intelligence Multi-Fichiers",
        "tracker": "📅 Suivi des Tâches & Jalons",
        "sandbox": "💡 Bac à Sable Remue-Méninges",
        "predictor": "💸 Prédiction de Ressources",
        "indexer": "🗂️ Indexeur de Base de Connaissances",
        "runway_plan": (
            "🛡️ Planificateur de Trésorerie & Réinvestissement"
        ),
        "sporty_ai": "⚽ Sporty Teaching AI",
        "forex_ai": "📈 Forex Teaching AI",
        "about_panel": "ℹ️ À Propos de l'App",
        "run_diag": "⚡ Lancer le Diagnostic Système",
        "error_ingest": (
            "🚨 Ingestion Manquante: Déposez les documents d'actifs d'abord."
        ),
        "calc_btn": "📊 Calculer les Paramètres",
        "success_ac": "⚡ Accélérateur de données mis à jour!",
        "save_fn": "Enregistrer sous (Nom du fichier):",
        "dl_lbl": "📥 Télécharger le fichier",
        "metrics_lbl": "Entrez les valeurs séparées par des virgules:",
    },
}

st.set_page_config(page_title="Vektor AI", page_icon="playstore.png")
...
with col_lang2:
  selected_language = st.selectbox(
      "🌐 GLOBAL TRANSLATION LANGUAGE",
      ["English", "Français"],
      index=0,
      key="global_lang_selection_bar",
  )
...
with col_lang1:
  ...
  current_time_str = datetime.now(ZoneInfo("Africa/Lagos")).strftime(
      "%Y-%m-%d %H:%M:%S"
  )
  clock_placeholder.markdown(
      f"<div class='live-clock'>🕒 SYSTEM TIME: {current_time_str}</div>",
      unsafe_allow_html=True,
  )
  ...
  with center_col:
    ...
    with auth_mode[0]:
      ...
      if st.button(tr["btn_auth"], use_container_width=True):
        vault = load_vault()
        if (
            encode_cred(input_user) in vault
            and vault[encode_cred(input_user)] == encode_cred(input_pass)
        ):
          ...
          if u_key in metrics:
            ...
            if expiry_time_str:
              ...
              ) > datetime.strptime(expiry_time_str, "%Y-%m-%d %H:%M:%S").replace(
            ...
            if is_unpaid and is_expired:
              ...
              st.write(
                  "Please paste a valid structural verification activation key"
                  " code below to update your operational window matrix."
              )

              activation_input = st.text_input(
                  "Enter Activation PIN (e.g., VK-XXXX-XXXX):",
                  key="activation_pin_box",
              )
              if st.button(
                  "🔓 Authenticate & Apply Key", use_container_width=True
              ):
                pins_db = load_pins()
                if (
                    activation_input in pins_db
                    and pins_db[activation_input]["status"] == "Unused"
                ):
                  ...
                  if pin_details["is_forever"]:
                    ...
                    new_expiry = datetime.now(
                        ZoneInfo("Africa/Lagos")
                    ) + timedelta(days=days_extension)
                    metrics[u_key]["license_expiry"] = new_expiry.strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                  ...
                  save_admin_metrics(metrics)
                  st.success(
                      "🎉 Activation Successful! Access Window Granted. Please"
          ...
          # Store login credentials persistently to browser cookies so user stays logged in
      ...
      if st.button(tr["btn_reg"], use_container_width=True):
        if new_user.strip() != "" and len(new_pass) >= 4:
          ...
          if encode_cred(new_user) in vault:
            ...
            st.success(
                "Workspace Provisioned! You have been granted a 1-Week Free"
                " Trial balance."
            )
  st.stop()
...
def query_standalone_engine(prompt):
  if not client:
    return (
        "🚨 API Key Missing: GROQ_API_KEY is not configured in environment"
        " variables."
    )
  try:
...
with st.sidebar:
  st.markdown(
      "<div class='cyber-logo' style='font-size: 1.8rem;'>⚡ VK-CORE</div>",
      unsafe_allow_html=True,
  )
  ...
  current_export_payload = (
      f"Vektor AI Operational Log Archive\nUser:"
      f" {st.session_state.current_user}\nModule Target:"
      f" {module_selection}\n---\n"
  )
  if module_selection == tr["oracle_chat"]:
    current_export_payload += json.dumps(
        st.session_state.get("chat_history", []), indent=2
    )
  ...
  if st.button(tr["logout"], use_container_width=True):
    ...
    if u_key in metrics:
      ...
      save_admin_metrics(metrics)
    # Flush the browser cookie securely so session truly ends only on logout button click
...
if user_encoded_key in metrics_db:
  ...
  if exp_date_str:
    exp_datetime = datetime.strptime(
...
if st.session_state.active_view == "NOTEPAD":
  st.markdown(
      "<div class='cyber-logo'>📝 SECURE WORKSPACE NOTEPAD STORAGE</div>",
      unsafe_allow_html=True,
  )
  ...
  box_height = st.slider(
      "📐 Adjust Custom Workspace Height (Pixels):", 200, 800, 400, step=50
  )
  notepad_input = st.text_area(
      "Write updates, code files, or business strategy details:",
      value=current_text,
      height=box_height,
  )
  ...
  if saved_history:
    for index, historical_note in enumerate(saved_history[:5]):
      with st.expander(
          f"Log Memory File #{index + 1} — Snippet:"
          f" {historical_note[:40]}..."
      ):
        st.code(historical_note)
        if st.button(
            f"🔄 Reload Log File #{index + 1} Into Main Text Box",
            key=f"reload_note_{index}",
        ):
          save_notepad(
              st.session_state.current_user, historical_note, saved_history
          )
...
# SCREEN 2: MULTIMODAL MEDIA FOUNDRY & VISION STUDIO
  ...
  st.markdown(
      "<div class='cyber-logo'>🎨 MULTIMODAL MEDIA CONTENT FOUNDRY & VISION"
      " CAMERA</div>",
      unsafe_allow_html=True,
  )
  st.caption(
      "High-Fidelity Real Image Generation, Text-to-Audio Scripting, and Photo"
      " Manipulation Matrix"
  )
  ...
  with col_m1:
    creative_prompt = st.text_input(
        "Enter generation description details:",
        placeholder=(
            "e.g., Ultra-modern corporate high-rise office in Lagos, cyberpunk"
            " aesthetics"
        ),
    )
  with col_m2:
    media_output_type = st.radio(
        "Output Modality Targets:",
        ["Render Real AI Image", "Generate Audio voiceover Script"],
    )

  if st.button("🔥 Initialize Media Generation Engine", use_container_width=True):
    if creative_prompt.strip() != "":
      st.markdown(
          "<div class='scanning-line'></div>", unsafe_allow_html=True
      )
      if media_output_type == "Render Real AI Image":
        ...
        st.markdown(
            "<div class='feature-card'><h4>🖼️ Output Image Generated Result"
            " Matrix:</h4></div>",
            unsafe_allow_html=True,
        )
        ...
        audio_desc_prompt = (
            "Convert this description into a highly detailed voiceover text"
            " script with technical audio sound design cues:"
            f" '{creative_prompt}'"
        )
        simulated_audio_script = query_standalone_engine(audio_desc_prompt)
        st.markdown(
            f"<div class='feature-card'><h4>🔊 Immersive Audio Description"
            f" Script:</h4><br>{simulated_audio_script}</div>",
            unsafe_allow_html=True,
        )
  ...
  camera_picture = st.camera_input(
      "Position the item or face in front of your lens:"
  )
  ...
  if camera_picture:
    ...
    with col_ed2:
      ...
      st.image(
          edited_img,
          caption="Edited Result Preview Target",
          use_container_width=True,
      )
    ...
    vision_instruction = st.text_input(
        "What information would you like the AI to extract or evaluate about"
        " this item?",
        value=(
            "Examine what this object/document is and provide an operational"
            " or industrial breakdown profile."
        ),
    )
    ...
    if st.button("🔍 Run Diagnostic Image Analysis", use_container_width=True):
      ...
      vision_analysis = query_standalone_engine(
          f"The user has captured an asset photo measuring"
          f" {edited_img.width}x{edited_img.height} pixels using color filter"
          f" template '{color_mode}'. Based on their prompt request:"
          f" '{vision_instruction}', provide an extensive professional"
          f" evaluation, technical specification insights, and strategic"
          f" recommendations written in language {st.session_state.lang}."
      )
      st.markdown(
          f"<div class='feature-card'><h4>📡 Vision Node Diagnostic"
          f" Report:</h4><br>{vision_analysis}</div>",
          unsafe_allow_html=True,
      )
  st.stop()
...
# SCREEN 3: BILLING METHOD & LICENSE EXTENSION PORTAL
  ...
  st.markdown(
      "<div class='cyber-logo'>💳 CORE BILLING NODES & LICENSE"
      " ACCELERATOR</div>",
      unsafe_allow_html=True,
  )
  st.caption(
      "Provision Payments, Redeem Structural Activation Invoices, and View"
      " Account Lifespan Matrices"
  )
  ...
  with col_b1:
    ...
    user_meta_profile = load_admin_metrics().get(
        encode_cred(st.session_state.current_user), {}
    )

    st.write(f"**Account Identity Space:** {st.session_state.current_user}")
    st.write(
        "**Current Structural Tier Status:**"
        f" `{user_meta_profile.get('payment_status', 'Unpaid')}`"
    )
    st.write(
        "**Computation Lifespan End Threshold:**"
        f" `{user_meta_profile.get('license_expiry', 'N/A')}`"
    )
  ...
  manual_invoice_pin = st.text_input(
      "Input Generated Administration Activation Key (PIN):",
      placeholder="e.g., VK-XXXX-XXXX",
  )
  if st.button("🔓 Submit Registration Renewal Block", use_container_width=True):
    if manual_invoice_pin.strip() != "":
      ...
      if (
          manual_invoice_pin in pins_database
          and pins_database[manual_invoice_pin]["status"] == "Unused"
      ):
        ...
        pins_database[manual_invoice_pin]["claimed_by"] = (
            st.session_state.current_user
        )
        ...
        if pin_info["is_forever"]:
          ...
          if current_exp_str and datetime.strptime(
              current_exp_str, "%Y-%m-%d %H:%M:%S"
          ).replace(tzinfo=ZoneInfo("Africa/Lagos")) > datetime.now(
              ZoneInfo("Africa/Lagos")
          ):
          ...
          all_metrics[user_hashed_key]["license_expiry"] = (
              new_computed_exp.strftime("%Y-%m-%d %H:%M:%S")
          )
          all_metrics[user_hashed_key]["payment_status"] = "Unpaid"

        save_admin_metrics(all_metrics)
        st.success(
            "🎉 Matrix Timeline Extension Applied! Click below to return."
        )
        time.sleep(1)
        st.rerun()
      else:
...
# SCREEN 4: DEDICATED FULLSCREEN ABOUT APPLICATION VIEW
  ...
  st.markdown(
      "<div class='cyber-logo'>ℹ️ ABOUT VEKTOR AI</div>",
      unsafe_allow_html=True,
  )
  st.caption(
      "Sovereign Information Portal — System Blueprint & Operations Manual"
  )
  ...
  st.info(
      "Vektor AI is a cutting-edge computing ecosystem engineered to streamline"
      " workflows, data management, and AI execution blocks. It bridges deep"
      " technical capability with functional, daily-use developer utilities,"
      " empowering users with instant command access through a unified local"
      " interface."
  )
  ...
  st.markdown(
      """
        1. **Core Engine Control**: Use the drop-down box at the top of the left sidebar to cycle through modules like the *Oracle Chat*, *Executive Briefing Engine*, *Sporty Teaching AI*, or *Forex Teaching AI*.
  ...
  st.markdown(
      """
        ...
        * **Sporty & Forex Teaching AI**: Interactive coaching and market analysis modules equipped with image uploads and live camera snapshot support for tactical and chart breakdowns.
...
with col_panel1:
  if st.button(
      "📝 Open Fullscreen Resizable Notepad Engine", use_container_width=True
  ):
    st.session_state.active_view = "NOTEPAD"
    st.rerun()
with col_panel2:
  if st.button(
      "🎨 Open Camera Snap & AI Image Content Foundry", use_container_width=True
  ):
    st.session_state.active_view = "FOUNDRY"
    st.rerun()
with col_panel3:
...
with st.container():
  ...
  with col_s1:
    search_item = st.text_input(
        "Look up global commodity pricing:",
        placeholder=tr["search_ph"],
        key="g_search_bar",
    )
  with col_s2:
    target_currency = st.text_input(
        tr["curr_lbl"], value="₦ (NGN)", key="g_curr_bar"
    )

  if st.button(tr["search_btn"], use_container_width=True):
    if search_item.strip() != "":
      st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
      res = query_standalone_engine(
          f"As an economy analyst, find the current global market pricing of"
          f" '{search_item}' and convert it into {target_currency}. Provide a"
          f" detailed breakdown in {st.session_state.lang}."
      )
      st.markdown(
          f"<div class='feature-card'><b>{tr['orac_res']}</b><br><br>{res}</div>",
          unsafe_allow_html=True,
      )

st.write("---")
uploaded_files = st.file_uploader(
    tr["upload_lbl"], type=["pdf"], accept_multiple_files=True
)

if uploaded_files:
  if "document_dict" not in st.session_state or len(
...
context_ready = (
    "raw_context" in st.session_state and st.session_state.raw_context.strip() != ""
)


# ==========================================
# VISION HELPER FOR SPORTY & FOREX
# ==========================================
...
if module_selection == tr["comp_panel"]:
  ...
  with st.expander("🛠 Rose Graph Configuration Studio", expanded=True):
    ...
    with col_g1:
      chart_type = st.selectbox(
          "Select Display Style:", ["Line Chart", "Bar Chart", "Area Chart"]
      )
      x_label = st.text_input("Label X-Axis:", value="Timeline")
    with col_g2:
      y_label = st.text_input("Label Y-Axis:", value="Performance")
      custom_data_points = st.text_input(
          tr["metrics_lbl"], value="10, 25, 18, 35, 42, 38, 50"
      )
  try:
    data_list = [
        float(i.strip()) for i in custom_data_points.split(",") if i.strip() != ""
    ]
    chart_data = pd.DataFrame(
        {x_label: range(1, len(data_list) + 1), y_label: data_list}
    ).set_index(x_label)
  ...
  user_query = st.text_input(
      "Query data context:", placeholder="What are our core operational findings?"
  )
  if user_query:
    if not context_ready:
      ...
      save_history(
          st.session_state.current_user, "chat", st.session_state.chat_history
      )

  if st.session_state.get("chat_history"):
    for q, a in reversed(st.session_state.chat_history):
      st.markdown(
          f"<div class='feature-card'><b>❓ Query:</b>"
          f" {q}<br><br><b>🤖 Answer:</b></div>",
          unsafe_allow_html=True,
      )
      st.markdown(a)
  ...
  if st.button(tr["run_diag"], use_container_width=True):
    if not context_ready:
      ...
      st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
      prompt = (
          f"Using this text context:\n{st.session_state.raw_context[:4000]}\n\nProvide"
          f" an executive briefing in language {st.session_state.lang}: 3 bullet"
          " summaries, key risk factors, and deadlines."
      )
      st.session_state.briefing_store = query_standalone_engine(prompt)
      save_history(
          st.session_state.current_user,
          "briefing",
          st.session_state.briefing_store,
      )
  ...
  style = st.selectbox(
      "Style:", ["Professional Email", "Formal Memorandum", "Slack Alert"]
  )
  ...
  if st.button(tr["calc_btn"], use_container_width=True):
    if not context_ready:
      ...
      prompt = (
          f"Context:\n{st.session_state.raw_context[:4000]}\n\nCompose a {style}"
          f" in language {st.session_state.lang}. Special Instruction: {notes}"
      )
      st.session_state.draft_store = query_standalone_engine(prompt)
      save_history(
          st.session_state.current_user, "draft", st.session_state.draft_store
      )
  ...
  if st.button(tr["calc_btn"], use_container_width=True):
    if not context_ready:
      ...
      prompt = (
          f"Context text:\n{st.session_state.raw_context[:4000]}\n\nExtract all"
          f" information relating to {target} in language"
      ...
      save_history(
          st.session_state.current_user,
          "structure",
          st.session_state.structure_store,
      )
  ...
  if not context_ready or len(st.session_state.get("document_dict", {})) < 2:
    st.warning(
        "⚠️ Cross-File operations require at least two PDF files uploaded"
        " simultaneously."
    )
  else:
    audit_query = st.text_input(
        "Audit Query:", value="Are there contradictions across these items?"
    )
    if st.button(tr["calc_btn"], use_container_width=True):
      st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
      aggregated_context = ""
      ...
      prompt = (
          f"Analyze these files in language {st.session_state.lang} for"
          f" contradictions:\n{aggregated_context}\nDirective: {audit_query}"
      )
      st.session_state.cross_store = query_standalone_engine(prompt)
      save_history(
          st.session_state.current_user, "cross", st.session_state.cross_store
      )
  ...
  if st.button(tr["calc_btn"], use_container_width=True):
    if not context_ready:
      ...
      st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
      prompt = (
          f"Extract timelines and task actions in language"
          f" {st.session_state.lang} from:\n{st.session_state.raw_context[:4000]}"
      )
      st.session_state.tracker_store = query_standalone_engine(prompt)
      save_history(
          st.session_state.current_user,
          "tracker",
          st.session_state.tracker_store,
      )
  ...
  if st.button(tr["calc_btn"], use_container_width=True) and idea:
    st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
    prompt = (
        "Act as an expert risk auditor. Stress-test this business idea in"
        f" language {st.session_state.lang}: '{idea}'"
    )
    st.session_state.sandbox_store = query_standalone_engine(prompt)
    save_history(
        st.session_state.current_user, "sandbox", st.session_state.sandbox_store
    )
  ...
  project_brief = st.text_area(
      "Describe the project scope & technical requirements:"
  )
  col_cfg1, col_cfg2 = st.columns(2)
  with col_cfg1:
    tier_profile = st.selectbox(
        "Strategy Profile:",
        [
            "Lean Bootstrap Strategy",
            "Standard Competitive Infrastructure",
            "Aggressive Scale Enterprise",
        ],
    )
  with col_cfg2:
  ...
  if st.button(tr["calc_btn"], use_container_width=True) and project_brief:
    st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
    prompt = (
        f"Act as a tech project architect. Analyze brief: '{project_brief}'."
        f" Strategy: {tier_profile} with {contingency_buffer}% buffer. Report"
        f" structural ranges using currency {target_currency} written in"
    ...
    save_history(
        st.session_state.current_user,
        "predictor",
        st.session_state.predictor_store,
    )
  ...
  if st.button(tr["calc_btn"], use_container_width=True):
    if not context_ready:
      ...
      st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
      prompt = (
          "Generate a technical index and metadata taxonomy tags in language"
          f" {st.session_state.lang} for:\n{st.session_state.raw_context[:4000]}"
      )
      st.session_state.indexer_store = query_standalone_engine(prompt)
      save_history(
          st.session_state.current_user, "indexer", st.session_state.indexer_store
      )
  ...
  total_liabilities = st.number_input(
      "Enter Total Expenses/Liabilities:", value=0, step=100
  )
  if st.button(tr["calc_btn"], use_container_width=True):
    revenue_text = ""
    if financial_doc:
    ...
    st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
    prompt = (
        "Act as a virtual corporate CFO. Statement:"
        f" '{revenue_text[:2000]}'. Expenses: {total_liabilities} in"
        f" {target_currency}. Map items sold, calculate net remainder growth"
        " capital, and list step-by-step optimization tactics in language"
        f" {st.session_state.lang}."
    )
    st.session_state.runway_store = query_standalone_engine(prompt)
    save_history(
        st.session_state.current_user, "runway", st.session_state.runway_store
    )
...
elif module_selection == tr["sporty_ai"]:
  st.subheader("⚽ Sporty Teaching AI Analyzer")
  st.write(
      "Upload tactical diagrams or snap live pictures of game setups to get an"
      " expert coaching breakdown."
  )

  sport_option = st.radio(
      "Choose image input method for Sporty AI:",
      ("Upload Image", "Snap Photo"),
      key="sport_radio",
  )
  sport_image = None

  if sport_option == "Upload Image":
    sport_image = st.file_uploader(
        "Upload sports graphic/tactic",
        type=["jpg", "jpeg", "png"],
        key="sport_up",
    )
  else:
    sport_image = st.camera_input(
        "Snap a live photo of the play", key="sport_cam"
    )

  sport_prompt = st.text_input(
      "Additional instructions or questions about this play:",
      placeholder="e.g., Explain the positioning of the players here",
  )

  if st.button("Analyze Sports Tactics / Image", use_container_width=True):
    if sport_image is not None:
      try:
        with st.spinner("Analyzing sports tactic..."):
          base64_img = encode_uploaded_image(sport_image)
                      ...
                      "content": [
                          {
                              ...
                              "text": (
                                  ...
                                  if sport_prompt
                                      ...
                                      "Analyze this sports tactic or setup and"
                                      " break it down clearly for teaching"
                                      f" purposes in {st.session_state.lang}."
                                  )
                              ),
                          },
                          {
          ...
          st.success("Sports Breakdown Result:")
          st.write(chat_completion.choices[0].message.content)
      except Exception as e:
        st.error(f"Error: {e}")
    else:
      try:
        with st.spinner("Generating sports coaching breakdown..."):
          chat_completion = client.chat.completions.create(
              messages=[
                  {
                      "role": "system",
                      "content": (
                          "You are an expert sports coach and tactical analyst."
                          " Explain concepts clearly for teaching in"
                          f" {st.session_state.lang}."
                      ),
                  },
                  {
                      ...
                      "content": (
                          sport_prompt
                          if sport_prompt
                          else "Provide general tactical coaching insights."
                      ),
                  },
              ],
              model="llama-3.1-8b-instant",
...
elif module_selection == tr["forex_ai"]:
  st.subheader("📈 Forex Teaching AI Analyzer")
  st.write(
      "Upload market charts or snap pictures of setups to receive educational"
      " breakdowns and risk management concepts."
  )

  forex_option = st.radio(
      "Choose image input method for Forex AI:",
      ("Upload Chart", "Snap Chart Photo"),
      key="forex_radio",
  )
  forex_image = None

  if forex_option == "Upload Chart":
    forex_image = st.file_uploader(
        "Upload forex chart image",
        type=["jpg", "jpeg", "png"],
        key="forex_up",
    )
  else:
    forex_image = st.camera_input(
        "Snap a photo of the chart screen", key="forex_cam"
    )

  forex_prompt = st.text_input(
      "Specific question about this forex chart setup:",
      placeholder="e.g., Explain support/resistance or trend lines shown",
  )

  if st.button("Analyze Forex Chart / Setup", use_container_width=True):
    if forex_image is not None:
      try:
        with st.spinner("Processing chart visual..."):
          base64_img = encode_uploaded_image(forex_image)
          chat_completion = client.chat.completions.create(
              model="qwen/qwen3.6-27b",
              messages=[
                  {
                      ...
                      "content": [
                          {
                              ...
                              "text": (
                                  ...
                                  if forex_prompt
                                      ...
                                      "Break down this financial or forex"
                                      " chart for educational teaching"
                                      " purposes with safety/risk notes in"
                                      f" {st.session_state.lang}."
                                  )
                              ),
                          },
          ...
          st.success("Forex Educational Lesson:")
          st.write(chat_completion.choices[0].message.content)
      except Exception as e:
        st.error(f"Error: {e}")
    else:
      try:
        with st.spinner("Generating forex educational breakdown..."):
          chat_completion = client.chat.completions.create(
              messages=[
                  {
                      "role": "system",
                      "content": (
                          "You are a professional financial educator specializing"
                          " in Forex markets. Teach concepts safely, clearly,"
                          " and include risk management warnings in"
                          f" {st.session_state.lang}."
                      ),
                  },
                  {
                      ...
                      "content": (
                          forex_prompt
                          if forex_prompt
                          else "Explain foundational forex trading principles."
                      ),
                  },
              ],
              model="llama-3.1-8b-instant",
          )
          st.success("Forex Lesson Result:")
          st.write(chat_completion.choices[0].message.content)
      except Exception as e:
        st.error(f"Error: {e}")
...
st.caption(
    "Strategic Decision Engine for High-Net-Worth Portfolios and Enterprise"
    " Capital"
)
...
with st.container():
  ...
  with col_inv1:
    liquidity_reserve = st.number_input(
        "Core Capital Reserve ($)", value=50000000, step=5000000
    )
  ...
  with col_inv3:
    macro_shock_scenario = st.selectbox(
        "Simulate Global Macro Economic Shock:",
        [
  ...
  if st.button(
      "⚡ Run Portfolio Stress-Test & Core Diagnostics", use_container_width=True
  ):
    st.markdown("<div class='scanning-line'></div>", unsafe_allow_html=True)
    ...
    st.markdown(
        f"<div class='feature-card'><h3>📊 Tactical Risk Matrix Diagnostic"
        f" Output</h3><br>{risk_analysis}</div>",
        unsafe_allow_html=True,
    )
